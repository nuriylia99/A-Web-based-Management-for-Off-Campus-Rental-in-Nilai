...
<head>
	...
	<title>I-Rent Portal | New Bookings   </title>
...
<div id="print">
								<table border="1"  class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%"  >
									...
									<?php 
$bid=intval($_GET['bid']);
									$sql = "SELECT tblusers.*,tbllocation.LocationName,tblhouse.HouseTitle,tblbooking.FromDate,tblbooking.ToDate,tblbooking.message,tblbooking.HouseId as vid,tblbooking.Status,tblbooking.PostingDate,tblbooking.id,tblbooking.BookingNumber,
DATEDIFF(tblbooking.ToDate,tblbooking.FromDate) as totalnodays,tblhouse.PricePerMonth
									  from tblbooking join tblhouse on tblhouse.id=tblbooking.HouseId join tblusers on tblusers.EmailId=tblbooking.userEmail join tbllocation on tblhouse.HouseLocation=tbllocation.id where tblbooking.id=:bid";
$query = $dbh -> prepare($sql);
	...
	<h3 style="text-align:center; color:red">#<?php echo htmlentities($result->BookingNumber);?> Booking Details </h3>
		...
		<tr>
										...
										<tr>
											...
											<tr>											
											<th>House Name</th>
											<td><a href="edit-House.php?id=<?php echo htmlentities($result->vid);?>"><?php echo htmlentities($result->LocationName);?> , <?php echo htmlentities($result->HouseTitle);?></td>
											<th>Booking Date</th>
...
<tr>
	...
	<td><?php echo htmlentities($ppdays=$result->PricePerMonth);?></td>
</tr>
<tr>
